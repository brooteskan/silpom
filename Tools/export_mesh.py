...
def _export(paths, objects, initialize, reorder):
    ...
    # Stage both files directly in the destination directory. Moving metadata
    # out of TemporaryDirectory retains its owner-only Windows ACL, preventing
    # an Asset Processor running as the artist from reading the exported file.
    with staged_fbx(paths['fbx'].parent) as fbx, staged_fbx(paths['sidecar'].parent) as sidecar:
