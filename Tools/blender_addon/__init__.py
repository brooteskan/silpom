...
bl_info = {
    'name': 'Curved SilPOM Exporter',
    'author': 'SilPOM contributors',
    'version': (0, 1, 0),
    'blender': (5, 0, 0),
    'location': 'File > Export; 3D View > Sidebar > SilPOM',
    'description': 'Tag curved displacement regions and export FBX with SilPOM metadata',
    'warning': 'Experimental transport only; curved runtime rendering is not yet available',
...
class SILPOM_OT_export(bpy.types.Operator, ExportHelper):
    ...
    def draw(self, _context):
        ...
        layout.label(text='Experimental: no curved runtime rendering', icon='INFO')
...
class SILPOM_PT_authoring(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text='Curved runtime rendering is not available')
        ...
        layout.separator()
        layout.operator('export_scene.silpom', text='Export Curved SilPOM', icon='EXPORT')
        layout.label(text='Requires UVs, materials and baked modifiers')


def menu_export(self, _context):
    self.layout.operator(SILPOM_OT_export.bl_idname, text='Curved SilPOM (.fbx + .json)')
