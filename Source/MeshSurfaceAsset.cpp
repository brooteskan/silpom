...
void MeshAssetSystem::Reflect(AZ::ReflectContext* context)
{
    ...
    MeshRenderFeatureProcessor::Reflect(context);
    if (auto* sc = azrtti_cast<AZ::SerializeContext*>(context))
...
void MeshAssetSystem::Activate()
{
    ...
    m_handler->Register();
    if (auto factory = AZ::RPI::FeatureProcessorFactory::Get()) factory->RegisterFeatureProcessor<MeshRenderFeatureProcessor>();
}
void MeshAssetSystem::Deactivate()
{
    if (auto factory = AZ::RPI::FeatureProcessorFactory::Get()) factory->UnregisterFeatureProcessor<MeshRenderFeatureProcessor>();
    m_handler.reset();
}
}
