...
{
    const Bivariate uvX = Affine(triangle.uv[0].x, triangle.uv[1].x - triangle.uv[0].x,
        triangle.uv[2].x - triangle.uv[0].x);
    const Bivariate uvY = Affine(triangle.uv[0].y, triangle.uv[1].y - triangle.uv[0].y,
        triangle.uv[2].y - triangle.uv[0].y);
    ...
    const Bivariate height = Constant(h00) + fx * (h10 - h00) + fy * (h01 - h00) +
        (fx * fy) * (h11 - h10 - h01 + h00);
    std::array<Bivariate, 3> position;
    for (int axis = 0; axis != 3; ++axis)
    {
        const Bivariate base = Affine(triangle.position[0][axis], triangle.position[1][axis] - triangle.position[0][axis],
            triangle.position[2][axis] - triangle.position[0][axis]);
        const Bivariate direction = Affine(triangle.direction[0][axis], triangle.direction[1][axis] - triangle.direction[0][axis],
            triangle.direction[2][axis] - triangle.direction[0][axis]);
