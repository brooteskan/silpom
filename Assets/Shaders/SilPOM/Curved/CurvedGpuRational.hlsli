...
struct SilPomCurved_ExactLinear {ExactValue a;ExactValue u;ExactValue v;};
SilPomCurved_ExactLinear SilPomCurved_ExactCorners(float a,float b,float c)
{
    SilPomCurved_ExactLinear result;result.a=ExactFloat(a);result.u=ExactSubtract(ExactFloat(b),result.a);
    result.v=ExactSubtract(ExactFloat(c),result.a);return result;
}
SilPomCurved_ExactLinear SilPomCurved_ExactLinearAdd(SilPomCurved_ExactLinear a,SilPomCurved_ExactLinear b)
{
    SilPomCurved_ExactLinear result;result.a=ExactAdd(a.a,b.a);result.u=ExactAdd(a.u,b.u);result.v=ExactAdd(a.v,b.v);return result;
}
SilPomCurved_ExactLinear SilPomCurved_ExactLinearScale(SilPomCurved_ExactLinear a,float scale)
{
    ExactValue s=ExactFloat(scale);a.a=ExactMultiply(a.a,s);a.u=ExactMultiply(a.u,s);a.v=ExactMultiply(a.v,s);return a;
}
struct SilPomCurved_ExactPolynomial {ExactValue value[6];}; // 1,u,v,uu,uv,vv
SilPomCurved_ExactPolynomial SilPomCurved_ExactSurfaceCoordinate(SilPomCurved_ExactLinear base,SilPomCurved_ExactLinear direction,SilPomCurved_ExactLinear height)
{
    SilPomCurved_ExactPolynomial result;
...
SilPomCurved_ExactPolynomial SilPomCurved_ExactProject(SilPomCurved_ExactPolynomial first,SilPomCurved_ExactPolynomial major,float origin,float majorOrigin,float direction,float majorDirection)
{
...
bool SilPomCurved_ExactIsLinear(SilPomCurved_ExactPolynomial p)
{
    return p.value[3].mantissa==0&&p.value[4].mantissa==0&&p.value[5].mantissa==0;
}
bool SilPomCurved_ExactPolynomialValid(SilPomCurved_ExactPolynomial p)
{
    bool valid=true;[unroll]for(uint i=0;i!=6;++i)valid=valid&&p.value[i].valid;return valid;
}
bool SilPomCurved_ExactClosedUnit(ExactValue value,ExactValue denominator)
{
...
bool SilPomCurved_ExactRatio(ExactValue numerator,ExactValue denominator,out float2 enclosure)
{
    enclosure=0;
    ...
    float2 ni=float2(SilPomCurved_ProofDown(n),SilPomCurved_ProofUp(n)),di=float2(SilPomCurved_ProofDown(d),SilPomCurved_ProofUp(d));
    if(di.x<=0&&di.y>=0)return false;
    precise float low=1.0f/di.y,high=1.0f/di.x;
    float2 reciprocal=float2(SilPomCurved_ProofDown(low),SilPomCurved_ProofUp(high));
    float scale=asfloat(uint(power+127)<<23);
    enclosure=SilPomCurved_ProofMultiply(SilPomCurved_ProofMultiply(ni,reciprocal),scale.xx);
    return all(isfinite(enclosure));
}

bool SilPomCurved_ExactLinearRange(ExactValue constant,ExactValue slope,ExactValue denominator,float2 u,out float2 range)
{
    float2 a,b;range=0;
    if(!SilPomCurved_ExactRatio(constant,denominator,a)||!SilPomCurved_ExactRatio(slope,denominator,b))return false;
    ...
    range=SilPomCurved_ProofAdd(a,SilPomCurved_ProofMultiply(b,u));return all(isfinite(range));
}

uint SilPomCurved_ProofQuadraticPair(SilPomCurved_Fragment f,SilPomCurved_Ray ray,SilPomCurved_Projection projection,SilPomCurved_ExactPolynomial position,
    ExactValue a,ExactValue b,ExactValue c,ExactValue discriminant,ExactValue n0,ExactValue d,
    SilPomCurved_ExactLinear x,SilPomCurved_ExactLinear y,out float3 barycentric,out float2 depth)
{
    barycentric=0;depth=0;
    float2 disc,bi,denominator,vr;
    if(!SilPomCurved_ExactRatio(discriminant,ExactFloat(1),disc)||disc.x<=0||
        !SilPomCurved_ExactRatio(b,ExactFloat(1),bi)||!SilPomCurved_ExactRatio(ExactMultiply(ExactFloat(2),a),ExactFloat(1),denominator)||
        !SilPomCurved_ExactRatio(n0,d,vr))return 0;
    ...
    float2 root=float2(SilPomCurved_ProofDown(estimate),SilPomCurved_ProofUp(estimate));bool enclosed=false;
    [loop]for(uint i=0;i!=16;++i)
    {
        float2 lowSquare=SilPomCurved_ProofMultiply(root.xx,root.xx),highSquare=SilPomCurved_ProofMultiply(root.yy,root.yy);
        if(root.x>=0&&lowSquare.y<=disc.x&&highSquare.x>=disc.y){enclosed=true;break;}
        root=float2(max(0,SilPomCurved_ProofDown(root.x)),SilPomCurved_ProofUp(root.y));
    }
    if(!enclosed)return 0;
    precise float reciprocalLow=1/denominator.y,reciprocalHigh=1/denominator.x;
    float2 reciprocal=float2(SilPomCurved_ProofDown(reciprocalLow),SilPomCurved_ProofUp(reciprocalHigh));
    ...
    float2 ci,li,qi;
    if(!SilPomCurved_ExactRatio(tc,td,ci)||!SilPomCurved_ExactRatio(tb,td,li)||!SilPomCurved_ExactRatio(ta,td,qi))return 0;
    bool found=false;
    [unroll]for(uint side=0;side!=2;++side)
    {
        float2 u=SilPomCurved_ProofMultiply(SilPomCurved_ProofAdd(-bi.yx,side==0?-root.yx:root),reciprocal);
        float2 w=SilPomCurved_ProofSubtract(SilPomCurved_ProofSubtract(float2(1,1),u),vr);
        if(u.y<0||u.x>1||w.y<0||w.x>1)continue;
        if(u.x<0||u.y>1||w.x<0||w.y>1)return 0;
        float2 cx,cy;
        if(!SilPomCurved_ExactLinearRange(ExactAdd(ExactMultiply(x.a,d),ExactMultiply(x.v,n0)),ExactMultiply(x.u,d),d,u,cx)||
            !SilPomCurved_ExactLinearRange(ExactAdd(ExactMultiply(y.a,d),ExactMultiply(y.v,n0)),ExactMultiply(y.u,d),d,u,cy))return 0;
        if(cx.y<0||cx.x>1||cy.y<0||cy.x>1)continue;
        if(cx.x<0||cx.y>1||cy.x<0||cy.y>1)return 0;
        float2 hit=SilPomCurved_ProofDivide(SilPomCurved_ProofSubtract(SilPomCurved_ProofAdd(ci,SilPomCurved_ProofMultiply(u,SilPomCurved_ProofAdd(li,SilPomCurved_ProofMultiply(qi,u)))),
...
uint SilPomCurved_ProofRational(SilPomCurved_Fragment f,SilPomCurved_Ray ray,SilPomCurved_Projection projection,out float3 barycentric,out float2 depth,out bool singular)
{
    barycentric=0;depth=0;singular=false;
    if(f.height.w!=0)return 0;
    SilPomCurved_ExactLinear x=SilPomCurved_ExactLinearScale(SilPomCurved_ExactCorners(f.uv[0].x,f.uv[1].x,f.uv[2].x),float(textureWidth));
    SilPomCurved_ExactLinear y=SilPomCurved_ExactLinearScale(SilPomCurved_ExactCorners(f.uv[0].y,f.uv[1].y,f.uv[2].y),float(textureHeight));
    ...
    SilPomCurved_ExactLinear h=SilPomCurved_ExactLinearAdd(SilPomCurved_ExactLinearScale(x,f.height.y),SilPomCurved_ExactLinearScale(y,f.height.z));
    h.a=ExactAdd(h.a,ExactSubtract(ExactFloat(f.height.x),ExactFloat(reference)));h=SilPomCurved_ExactLinearScale(h,amplitude);
    SilPomCurved_ExactPolynomial positions[3];
    [unroll]for(uint axis=0;axis!=3;++axis)
        positions[axis]=SilPomCurved_ExactSurfaceCoordinate(SilPomCurved_ExactLinearScale(SilPomCurved_ExactCorners(f.position[0][axis],f.position[1][axis],f.position[2][axis]),baseScale),
            SilPomCurved_ExactCorners(f.direction[0][axis],f.direction[1][axis],f.direction[2][axis]),h);
    SilPomCurved_ExactPolynomial constraint=SilPomCurved_ExactProject(positions[projection.first],positions[projection.major],ray.origin[projection.first],
        ray.origin[projection.major],ray.direction[projection.first],ray.direction[projection.major]);
    SilPomCurved_ExactPolynomial q=SilPomCurved_ExactProject(positions[projection.second],positions[projection.major],ray.origin[projection.second],
        ray.origin[projection.major],ray.direction[projection.second],ray.direction[projection.major]);
    if(!SilPomCurved_ExactPolynomialValid(constraint)||!SilPomCurved_ExactPolynomialValid(q))return 0;
    if(!SilPomCurved_ExactIsLinear(constraint)){SilPomCurved_ExactPolynomial temporary=constraint;constraint=q;q=temporary;}
    if(!SilPomCurved_ExactIsLinear(constraint))return 0;
    ...
    else
    {
        ...
        if(discriminant.mantissa!=0)
        {
            ...
            return SilPomCurved_ProofQuadraticPair(f,ray,projection,positions[projection.major],a,b,c,discriminant,n0,d,x,y,barycentric,depth);
        }
    ...
    if(!SilPomCurved_ExactClosedUnit(u,denominator)||!SilPomCurved_ExactClosedUnit(v,denominator)||!SilPomCurved_ExactClosedUnit(w,denominator)||
        !SilPomCurved_ExactClosedUnit(cx,denominator)||!SilPomCurved_ExactClosedUnit(cy,denominator))return 2;
    SilPomCurved_ExactPolynomial t=positions[projection.major];
    ...
    float2 rootU,rootV;
    if(!SilPomCurved_ExactRatio(u,denominator,rootU)||!SilPomCurved_ExactRatio(v,denominator,rootV)||!SilPomCurved_ExactRatio(nt,dt,depth))return 0;
